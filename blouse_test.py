...
#     test_writer = tf.summary.FileWriter('/workspace/TC/Graph/blouse/test/', sess.graph)
    ...
    model_file=tf.train.latest_checkpoint('TC/model/blouse/')
    saver.restore(sess,model_file)
    inpath='TC/data/blouse/val/'
...
#         if i<200:
            with open(inpath+f,'r') as fid:
                dat=json.load(fid)
            pts=np.array(dat)
            if  pts[5,2]!=-1 and pts[6,2]!=-1:
                n=n+1
                x1=cv2.imread(inpath+f[0:-5]+'.jpg')
                scale=x1.shape[0]/256.0
                x2=cv2.resize(x1,(256,256))
                x3=cv2.cvtColor(x2,cv2.COLOR_BGR2RGB)
        ...
        #     print stage_heatmap_np[0].shape
                heatmap=stage_heatmap_np[0]+stage_heatmap_np[1]+stage_heatmap_np[2]
                pmap=cv2.resize(heatmap[0,:,:,:13],(256,256))
                pt=np.zeros((13,2))
                ...
                for k in range(13):
                    if pts[k,2]==1:
                        m=m+1
                        distance+=np.sqrt(np.sum(np.square(pts[k,:2]-pt[k])))
#                         cv2.circle(x1,center=(pt[k,0],pt[k,1]),radius=3,color=[255,0,0], thickness=-1)
                ne=distance/(m*np.sqrt(np.sum(np.square(pts[5,:2]-pts[6,:2]))))
                NE+=ne
                print(ne)
                
print('mean is {}'.format(NE/n))
